...
Module Info.
  ...
  Definition source_file := "src/console.c".
...
Definition _get_user_input : ident := $"get_user_input".
Definition _inbox : ident := $"inbox".
Definition _inbox_t : ident := $"inbox_t".
Definition _job : ident := $"job".
Definition _job_console : ident := $"job_console".
...
Definition _toggle_msg : ident := $"toggle_msg".
Definition _unit : ident := $"unit".
...
Definition f_job_console := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_tid, tschar) :: (_unit, (tptr tint)) :: (_pbt, tulong) ::
...
(Ssequence
  (Ssequence
    (Scall (Some _t'1)
      (Evar _get_user_input (Tfunction Tnil tint cc_default)) nil)
    (Sset _r (Etempvar _t'1 tint)))
  (Ssequence
    (Sifthenelse (Ebinop One (Etempvar _r tint)
                   (Econst_int (Int.repr 0) tint) tint)
      (Scall None
        (Evar _pals_send (Tfunction (Tcons tschar (Tcons (tptr tschar) Tnil))
                           tvoid cc_default))
        ((Econst_int (Int.repr 6) tint) ::
         (Evar _toggle_msg (tarray tschar 8)) :: nil))
      Sskip)
    (Scall None
...
((___builtin_ais_annot,
 ...
 (_pals_send,
   Gfun(External (EF_external "pals_send"
     ...
     tvoid cc_default)) ::
 (_get_user_input,
   Gfun(External (EF_external "get_user_input"
                   (mksignature nil AST.Tint cc_default)) Tnil tint
     cc_default)) ::
 (_write_log,
   Gfun(External (EF_external "write_log"
                   (mksignature (AST.Tint :: nil) AST.Tvoid cc_default))
     (Tcons tint Tnil) tvoid cc_default)) :: (_TASK_ID, Gvar v_TASK_ID) ::
 (_toggle_msg, Gvar v_toggle_msg) ::
 (_job_console, Gfun(Internal f_job_console)) ::
 (_job, Gfun(Internal f_job)) :: nil).

Definition public_idents : list ident :=
(_job :: _job_console :: _toggle_msg :: _TASK_ID :: _write_log ::
 _get_user_input :: _pals_send :: ___builtin_debug ::
