...
(* - Replace v_send_buf and v_mstore. *)
(* - Replace 23, 13, 15, 28 with *)
(*   __MAX_MSG_SIZE__, __MAX_NUM_TASKS__, __MAX_NUM_MCASTST, (__MAX_NUM_TASKS__ + __MAX_NUM_MCASTS__).  *)
(* - Insert section End and sanity checks. *)
...
Variable __MSG_SIZE_K__: Z.
Variable __MAX_NUM_TASKS__: Z.
Variable __MAX_NUM_MCASTS__: Z.

Let __MAX_MSG_SIZE__: Z := 8 * __MSG_SIZE_K__ + 7.
...
Definition msg_struct_sz: Z := (__MSG_SIZE_K__ + 2) * 8.

Definition v_send_buf := {|
  gvar_info := (Tstruct _pals_msg_t noattr);
  gvar_init := (Init_space msg_struct_sz :: nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.

Definition msg_entry_sz: Z := (__MSG_SIZE_K__ + 1) * 8.
Definition inbox_sz: Z := msg_entry_sz * __MAX_NUM_TASKS__.

Let msg_store_sz: Z := 4 + inbox_sz * 2.

Example msg_store_sz_ex
        (K_EQ: __MSG_SIZE_K__ = 1)
        (MAX_NT_EQ: __MAX_NUM_TASKS__ = 16)
  : msg_store_sz = 516.
Proof.
  unfold msg_store_sz, inbox_sz, msg_entry_sz.
  rewrite K_EQ, MAX_NT_EQ. simpl. reflexivity.
Qed.

Definition v_mstore := {|
  gvar_info := (Tstruct _msg_store_t noattr);
  gvar_init := (Init_space msg_store_sz :: nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.

Definition v_send_hist := {|
  gvar_info := (tarray tschar __MAX_NUM_TASKS__);
  gvar_init := (Init_space __MAX_NUM_TASKS__ :: nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.
...
(Ssequence
  (Sifthenelse (Ebinop Olt (Etempvar _id_dest tschar) (Evar _NUM_TASKS tint)
    ...
    (Sifthenelse (Ebinop Oeq
                   (Ederef
                     (Ebinop Oadd (Evar _send_hist (tarray tschar __MAX_NUM_TASKS__))
    ...
    (Sifthenelse (Ebinop Olt (Etempvar _id_dest tschar)
      ...
      (Ssequence
        (Sset _group_mem
          (Ederef
            (Ebinop Oadd (Evar _MCAST_MEMBER (tarray (tarray tschar __MAX_NUM_TASKS__) __MAX_NUM_MCASTS__))
              (Ebinop Osub (Etempvar _id_dest tschar) (Evar _NUM_TASKS tint)
                tint) (tptr (tarray tschar __MAX_NUM_TASKS__))) (tarray tschar __MAX_NUM_TASKS__)))
        (Ssequence
          (Ssequence
            (Sset _i (Econst_int (Int.repr 0) tint))
            (Sloop
              (Ssequence
                ...
                (Sifthenelse (Ebinop Olt (Econst_int (Int.repr 0) tint)
                  ...
                  (Sifthenelse (Ebinop Olt (Econst_int (Int.repr 0) tint)
                                 (Ederef
                                   (Ebinop Oadd
                                     (Evar _send_hist (tarray tschar __MAX_NUM_TASKS__))
...
(Ssequence
  ...
  (Sifthenelse (Etempvar _t'1 tint)
    (Ssequence
      (Sset _dest_ip
        (Ederef
          (Ebinop Oadd (Evar _IP_ADDR (tarray (tarray tschar 16) (__MAX_NUM_TASKS__ + __MAX_NUM_MCASTS__)))
        ...
        (Scall None
          ...
          ((Etempvar _msg (tptr tschar)) ::
           (Efield (Evar _send_buf (Tstruct _pals_msg_t noattr)) _content
             (tarray tschar __MAX_MSG_SIZE__)) :: nil))
        (Scall None
    ...
    (Ssequence
      ...
      (Ssequence
        (Sset _group_mem
          (Ederef
            (Ebinop Oadd (Evar _MCAST_MEMBER (tarray (tarray tschar __MAX_NUM_TASKS__) __MAX_NUM_MCASTS__))
              (Etempvar _i tint) (tptr (tarray tschar __MAX_NUM_TASKS__))) (tarray tschar __MAX_NUM_TASKS__)))
        (Sifthenelse (Ebinop Olt (Econst_int (Int.repr 0) tint)
                       (Ederef
                         (Ebinop Oadd (Etempvar _group_mem (tptr tschar))
                           (Evar _TASK_ID tschar) (tptr tschar)) tschar) tint)
          (Scall None
         ...
         (Efield
           (Ederef
             (Ebinop Oadd (Etempvar _me (tptr (Tstruct _msg_entry_t noattr)))
               (Etempvar _sender_id tschar)
               (tptr (Tstruct _msg_entry_t noattr)))
             (Tstruct _msg_entry_t noattr)) _content (tarray tschar __MAX_MSG_SIZE__)) ::
         nil)))
    Sskip))
|}.
    ...
    (Ssequence
      ...
      (Sset _cptr
        (Efield
          ...
          (tarray (Tstruct _msg_entry_t noattr) __MAX_NUM_TASKS__))))
    (Ssequence
      (Ssequence
        (Scall (Some _t'2)
                        ...
                        (Scall None
                          ...
                          ((Etempvar _cptr (tptr (Tstruct _msg_entry_t noattr))) ::
                           (Efield (Evar _buf (Tstruct _pals_msg_t noattr))
                             _sender tschar) ::
                           (Efield (Evar _buf (Tstruct _pals_msg_t noattr))
                             _content (tarray tschar __MAX_MSG_SIZE__)) :: nil))
                        (Sifthenelse (Ebinop Oeq
                                       (Efield
                                         (Evar _buf (Tstruct _pals_msg_t noattr))
                                         _period_base_time tulong)
            ...
            (Ebinop Oadd
              (Efield
                (Ederef (Etempvar _inb (tptr (Tstruct _inbox_t noattr)))
                  (Tstruct _inbox_t noattr)) _entry
                (tarray (Tstruct _msg_entry_t noattr) __MAX_NUM_TASKS__)) (Etempvar _i tint)
              (tptr (Tstruct _msg_entry_t noattr)))
            (Tstruct _msg_entry_t noattr)) _received tschar)
        (Econst_int (Int.repr 0) tint)))
    (Sset _i
...
(Composite _msg_entry_t Struct
   ((_received, tschar) :: (_content, (tarray tschar __MAX_MSG_SIZE__)) :: nil)
   noattr ::
 Composite _inbox_t Struct
   ((_entry, (tarray (Tstruct _msg_entry_t noattr) __MAX_NUM_TASKS__)) :: nil)
   noattr ::
...
Module SANITY_CHECK.
  ...
  Section CHECK1.
    ...
    Lemma prog_eq1
      : prog k max_nt max_nmc = main1.prog.
    Proof.
      reflexivity.
    Qed.
  End CHECK1.
  ...
  Section CHECK2.
    ...
    Lemma prog_eq2
      : prog k max_nt max_nmc = main2.prog.
    Proof.
      reflexivity.
    Qed.
  End CHECK2.
